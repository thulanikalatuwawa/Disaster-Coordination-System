...
public class ReportDisasterC {
    ...
    // report submitted with user inputs
    @FXML
    public void SubmitReport() {
        ...
        if (fullName.isEmpty() || contactNumber.isEmpty()|| nicNumber.isEmpty() || streetAddress.isEmpty() ||
                district == null || province == null || disaster == null || date == null || time == null ||urgencyLevelSelection == null) {
            Alert alert = new Alert(Alert.AlertType.ERROR, "Please fill the mandatory fields", ButtonType.OK);
            alert.show();
            return;
        }
        //submitted
        boolean isSuccess = reportService.submitDisasterReport(fullName, contactNumber, emailAddress, nicNumber, streetAddress, gramaNiladhariDivision,
                district, province, disaster, date, time, impactDescriptionText,noOfAffected, urgencyLevelSelection);
